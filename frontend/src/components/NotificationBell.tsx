...
function buildUserNotifications(userId: string, role: string): Notification[] {
    ...
    notes.push(
      {
        ...
        type: "system",
        title: "New User Registered",
        message: "Dr. Michael Chowdhury joined as doctor",
        time: new Date(Date.now() - 71 * 24 * 60 * 60000).toISOString(),
        read: false,
      },
      {
        id: "sys-newuser-2",
        type: "system",
        title: "New User Registered",
        message: "Dr. Sarah Jenkins joined as doctor",
        time: new Date(Date.now() - 75 * 24 * 60 * 60000).toISOString(),
        read: false,
      },
    );
  }
  ...
  notes.push(
    ...
    },
    {
      ...
      message:
        "Book consultations, manage prescriptions, and track payments all in one place.",
...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    const notes = buildUserNotifications(userId, role);
  ...
  // Always show NotificationBell, returning guest notifications when no user is logged in
  ...
  return (
    <div ref={ref} className="relative flex items-center">
      ...
      <button
        onClick={handleOpen}
        className="keep-rounded relative flex items-center justify-center p-2 rounded-full hover:bg-slate-100 dark:hover:bg-white/10 text-slate-500 dark:text-zinc-350 hover:text-indigo-600 dark:hover:text-rose-500 transition-colors cursor-pointer"
        aria-label="Notifications"
      >
        <Bell className="h-5 w-5" />
        {unread > 0 && (
          <span className="absolute top-0.5 right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-rose-600 text-[9px] font-extrabold text-white shadow-sm animate-pulse">
            {unread > 9 ? "9+" : unread}
          </span>
        )}
      </button>
        ...
        <div className="absolute right-0 top-full mt-2 w-[360px] max-h-[520px] origin-top-right rounded-2xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-2xl shadow-black/20 z-[60] flex flex-col overflow-hidden animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="flex items-center justify-between px-4 py-3.5 border-b border-slate-100 dark:border-zinc-800 shrink-0">
            ...
            <div className="flex items-center gap-1">
              {unread > 0 && (
                <button
                  onClick={markAllRead}
                  className="flex items-center gap-1 text-[11px] font-bold text-rose-600 dark:text-rose-500  hover:underline px-2 py-1 rounded-lg hover:bg-rose-500/10 transition-colors cursor-pointer"
                >
              ...
              notifications.map((n) => {
                ...
                return (
                  ...
                  >
                    {/* Icon */}
                    <div
                      className={`h-9 w-9 rounded-xl ${cfg.bg} flex items-center justify-center shrink-0 mt-0.5`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <span
                          className={`text-xs font-bold leading-tight ${isRead ? "text-slate-700 dark:text-zinc-300" : "text-slate-900 dark:text-zinc-50"}`}
                        >
                          {n.title}
                        </span>
                        {!isRead && (
                          <span
                            className={`mt-0.5 h-2 w-2 rounded-full ${cfg.badge} shrink-0`}
                          />
                        )}
                      </div>
          ...
          <div className="px-4 py-3 border-t border-slate-100 dark:border-zinc-800 shrink-0 bg-slate-50/50 dark:bg-zinc-800/20">
            <p className="text-[10px] text-slate-400 dark:text-zinc-500 text-center font-medium">
              ...
              {notifications.length !== 1 ? "s" : ""} · Medi-Doc Health System
            </p>
          </div>
        </div>
      )}
