  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data && data.length > 0) {
            const normalized = data.map((item: any, index: number) => ({
              ...
              patientName:
                item.patientName || item.userName || "Anonymous Patient",
              patientPhoto:
                item.patientPhoto ||
              ...
              reviewText:
                item.reviewText ||
                item.comment ||
                "Excellent healthcare experience at MediCare Connect.",
        ...
        console.log(
          "Backend reviews fetch failed, falling back to local DB:",
          err,
        );
      }
        ...
        <div className="container mx-auto px-0 max-w-7xl space-y-10 rounded-none">
          ...
          <div className="space-y-3">
            ...
            <p className="text-base text-slate-650 dark:text-zinc-400 leading-relaxed font-semibold max-w-3xl">
              Hear from our patients who have experienced quick booking,
              personalized specialist care plans, and secure online health
                  ...
                  <div className="relative bg-slate-50 dark:bg-zinc-900 border-none p-6 flex flex-col justify-between h-[340px] lg:h-[310px] shadow-xs hover:shadow-md hover:scale-[1.01] transition-all duration-300 rounded-[12px] overflow-hidden group">
                    ...
                    <div className="flex items-center gap-3 pt-4 border-t border-slate-200/50 dark:border-zinc-800/80">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="inline-flex items-center gap-1 rounded bg-rose-500/10 px-1.5 py-0.5 text-[8px] font-bold text-rose-600 dark:text-rose-500  uppercase tracking-wide mt-0.5">
          ...
          {reviews.length > 0 && (
            <div className="flex items-center justify-center gap-6 pt-2">
              ...
              <div className="flex items-center gap-2">
                {Array.from({ length: Math.min(reviews.length, 6) }).map(
                  (_, idx) => (
                    <div
                      key={idx}
