  ...
  useEffect(() => {
    ...
    if (foundUser) {
      ...
      if (foundUser.role === "doctor") {
        const foundDoc = db
          .getDoctors()
          .find((d) => d.userId === foundUser.id || d.id === foundUser.id);
        if (foundDoc) {
  ...
  return (
    <div className="container mx-auto px-6 py-10 max-w-[1100px] space-y-8">
      ...
      <div className="flex items-center gap-2 text-[10px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-widest">
        ...
        <span className="text-rose-600 dark:text-rose-500 ">Profile</span>
        <ChevronRight className="h-3 w-3" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="md:col-span-4 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm flex flex-col items-center p-6 text-center h-full">
          ...
          <span className="inline-flex items-center border border-rose-200 dark:border-rose-800/40 bg-rose-50/50 dark:bg-rose-950/20 px-3 py-1 rounded-full text-[10px] font-bold text-rose-700 dark:text-rose-500  uppercase tracking-wider mt-2.5">
            {profileUser.role}
          </span>
                ...
                <div className="p-6 space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="flex items-start gap-3 bg-slate-50 dark:bg-zinc-800/30 p-3.5 rounded-xl border border-slate-100 dark:border-zinc-800/30">
                      ...
                      <div>
                        <div className="text-xs font-bold text-slate-400 dark:text-zinc-550 uppercase tracking-widest">
                          Qualifications
                        </div>
                        <div className="text-base font-bold text-slate-800 dark:text-zinc-200 mt-1">
                    ...
                    <div className="flex items-start gap-3 bg-slate-50 dark:bg-zinc-800/30 p-3.5 rounded-xl border border-slate-100 dark:border-zinc-800/30">
                      ...
                      <div>
                        ...
                        <div className="text-base font-extrabold text-slate-800 dark:text-white mt-1">
                  ...
                  {profileDoctor.verificationStatus === "verified" && (
                    <div className="flex items-center gap-2 bg-rose-500/5 border border-rose-500/10 rounded-xl px-4 py-3 text-rose-600 dark:text-rose-500  text-xs font-semibold">
                      <CheckCircle className="h-4.5 w-4.5 text-rose-600" />
                      <span>
                        This doctor is fully verified and licensed by Medi-Doc
                        Health Systems.
                      </span>
                    </div>
                  )}
                  ...
                  <div className="border-t border-slate-100 dark:border-zinc-800/60 pt-6 space-y-4">
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                        <div className="text-xs font-bold text-slate-500 dark:text-zinc-400 w-24 shrink-0">
                          Available Days:
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          {profileDoctor.availableDays.map((day) => (
                            <span
                              key={day}
                              className="bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 px-2.5 py-1 rounded-md text-[10px] font-bold"
                            >
                              {day}
                            </span>
                          ))}
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {profileDoctor.availableSlots.map((slot) => (
                            <span
                              key={slot}
                              className="bg-rose-100/50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-500  px-2.5 py-1 rounded-md text-[10px] font-bold flex items-center gap-1"
                            >
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-base font-semibold text-slate-700 dark:text-zinc-300">
                  <div className="flex justify-between py-3 border-b border-slate-100 dark:border-zinc-800/40">
                    ...
                    <span className="capitalize font-bold text-slate-800 dark:text-zinc-100">
                      {profileUser.role}
                    </span>
                  </div>
                  <div className="flex justify-between py-3 border-b border-slate-100 dark:border-zinc-800/40">
                    <span className="text-slate-400 font-medium">Status</span>
                    <span className="text-rose-600 dark:text-rose-500  font-bold capitalize">
                      {profileUser.status}
                    </span>
                  </div>
                  <div className="flex justify-between py-3 border-b border-slate-100 dark:border-zinc-800/40">
                ...
                <div className="bg-rose-500/5 border border-rose-500/10 rounded-xl p-4 flex items-start gap-3 mt-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-zinc-400 mt-1 leading-relaxed">
                      This user holds a patient registry account in the Medi-Doc
                      system. All clinical appointments, prescription
                      directories, and lab diagnostics are encrypted and stored
                      within the patient's secure dashboard.
                    </p>
                  </div>
                </div>
              </div>
