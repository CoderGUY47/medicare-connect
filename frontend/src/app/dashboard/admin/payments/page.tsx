...
import { FiChevronRight, FiDollarSign, FiTrendingUp, FiCreditCard, FiDownload } from 'react-icons/fi';
...
export default function AdminPaymentsPage() {
  ...
  const avgTx = payments.length > 0 ? Math.round(totalRevenue / payments.length) : 0;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 ml-12">
            All consultation fee receipts, transaction logs, and revenue analytics.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-sm font-bold text-slate-800 dark:text-zinc-100">Monthly Revenue</h3>
              <p className="text-xs text-slate-400 mt-0.5">Consultation fee revenue by month</p>
            </div>
            <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-lg">2026</span>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
          <div className="mb-6">
            <h3 className="text-sm font-bold text-slate-800 dark:text-zinc-100">By Doctor</h3>
            <p className="text-xs text-slate-400 mt-0.5">Revenue per specialist</p>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart
              data={(() => {
                const byDoc: Record<string, number> = {};
                payments.forEach(p => { byDoc[p.doctorName.replace('Dr. ', '')] = (byDoc[p.doctorName.replace('Dr. ', '')] || 0) + p.amount; });
                return Object.entries(byDoc).map(([name, val]) => ({ name, revenue: val }));
              })()}
      ...
      {payments.length > 0 ? (
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between">
            <h2 className="text-sm font-bold text-slate-800 dark:text-zinc-100">Transaction Ledger</h2>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
                {payments.map((p, i) => (
                  <tr key={p.id} className="hover:bg-rose-50/30 dark:hover:bg-zinc-800/40 transition-colors">
                    <td className="px-5 py-4 text-[10px] font-bold text-slate-400 dark:text-zinc-500">{String(i + 1).padStart(2, '0')}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 dark:text-zinc-400 font-mono whitespace-nowrap">{p.transactionId}</span>
                      </div>
                    </td>
          ...
          <div className="px-6 py-3.5 border-t border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-800/20 flex items-center justify-between">
            ...
            <div className="text-sm font-extrabold text-slate-700 dark:text-zinc-200">
              Total: <span className="text-emerald-600 dark:text-emerald-400">${totalRevenue.toLocaleString()}.00</span>
            </div>
          </div>
        </div>
      ) : (
        <div className="bg-white dark:bg-zinc-900 border border-dashed border-slate-300 dark:border-zinc-700 rounded-2xl py-20 flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-sm font-bold text-slate-500 dark:text-zinc-400">No transactions recorded yet</p>
        </div>
      )}
    </div>
  );
