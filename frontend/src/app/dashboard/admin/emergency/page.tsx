...
const MOCK_CASES: EmergencyCase[] = [
  {
    ...
    age: 62,
    gender: "Male",
    complaint: "Acute chest pain, elevated troponin",
    severity: "critical",
    admittedAt: "2026-06-20 08:14",
  ...
  },
  {
    ...
    age: 34,
    gender: "Female",
    complaint: "Seizure episode, post-ictal confusion",
    severity: "critical",
    admittedAt: "2026-06-20 09:02",
  ...
  },
  {
    ...
    age: 47,
    gender: "Male",
    complaint: "Compound fracture, right femur",
    severity: "moderate",
    admittedAt: "2026-06-20 07:45",
  ...
  },
  {
    ...
    age: 28,
    gender: "Female",
    complaint: "Severe allergic reaction (anaphylaxis)",
    severity: "critical",
    admittedAt: "2026-06-20 10:30",
    assignedDoctor: "Dr. Sarah Jenkins",
    ward: "Resus Room",
    status: "active",
  },
  {
    ...
    age: 55,
    gender: "Male",
    complaint: "Hypertensive emergency (BP: 220/140)",
    severity: "moderate",
    admittedAt: "2026-06-20 06:55",
  ...
  },
  {
    ...
    age: 19,
    gender: "Female",
    complaint: "Laceration on forearm, requires sutures",
    severity: "stable",
    admittedAt: "2026-06-20 11:10",
    assignedDoctor: "Dr. Michael Chen",
    ward: "Minor Cases",
    status: "active",
  },
...
const STATUS_CFG = {
  active: {
    label: "Active",
    cls: "bg-rose-500/10 text-rose-600 dark:text-rose-500  border border-rose-500/20",
  },
  transferred: {
    label: "Transferred",
...
export default function EmergencyPage() {
  ...
  const handleDischarge = (id: string) => {
    setCases((prev) =>
      prev.map((c) => (c.id === id ? { ...c, status: "discharged" } : c)),
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 ml-12">
            Live emergency case board — active admissions and severity triage.
          </p>
        </div>
        {counts.critical > 0 && (
          <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 font-bold text-xs px-4 py-2.5 rounded-xl">
            ...
            {counts.critical} Critical Case{counts.critical > 1 ? "s" : ""}{" "}
            Active
          </div>
        )}
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      ...
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${sc.dot} ${sc.pulse ? "animate-pulse" : ""}`}
                        />
                        {sc.label}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-xs font-semibold text-slate-700 dark:text-zinc-200 whitespace-nowrap">
                      {c.ward}
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`inline-flex items-center text-[11px] font-bold px-2.5 py-1 rounded-lg ${stc.cls}`}
                      >
                        {stc.label}
                      </span>
                    </td>
                      ...
                      {c.status === "active" && (
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 hover:bg-blue-500/20 cursor-pointer transition-all"
                          >
                            Transfer
                          </button>
                          <button
                            onClick={() => handleDischarge(c.id)}
