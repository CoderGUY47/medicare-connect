...
import { RiAdminLine, RiNurseLine } from "react-icons/ri";
...
> = {
  admin: {
    ...
    icon: <RiAdminLine className="h-3 w-3" />,
  },
  doctor: {
    label: "Doctor",
  ...
  nurse: {
    ...
    icon: <RiNurseLine className="h-3 w-3" />,
  },
  lab: {
    label: "Lab Staff",
    ...
    icon: <MdOutlineScience className="h-3 w-3" />,
  },
  pharmacist: {
    label: "Pharmacist",
    ...
    icon: <MdOutlineLocalPharmacy className="h-3 w-3" />,
  },
};
  ...
  const handleToggleStatus = (userId: string, currentStatus: string) => {
    ...
    db.setUsers(
      db
        ...
        .map((u) =>
          u.id === userId
            ? { ...u, status: newStatus as "active" | "suspended" }
            : u,
        ),
    );
    loadUsers();
    ...
    <div className="space-y-7">
        ...
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-rose-500/10 flex items-center justify-center">
              <FiUsers className="h-5 w-5 text-rose-600 dark:text-rose-500 " />
            </div>
            Manage Accounts
          </h1>
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 ml-12">
            Control access, roles, and account status across all registered
            users.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
        {[
          ...
          <div
            key={stat.label}
            className={`bg-white dark:bg-zinc-900 border ${stat.border} rounded-2xl p-4 flex flex-col items-center justify-center gap-1 hover:shadow-md transition-shadow text-center`}
          >
                ...
                {filtered.map((u, idx) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          {u.photo ? (
                            ...
                            <div
                              className={`h-10 w-10 rounded-full bg-linear-to-br ${gradient} flex items-center justify-center text-white text-xs font-extrabold shrink-0`}
                            >
                              {initials(u.name)}
                            </div>
                          )}
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-lg border ${rc.color} ${rc.bg} ${rc.border}`}
                        >
                          {rc.icon}
                          {rc.label}
                        </span>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="relative">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                setDropdownOpen(isOpen ? null : u.id);
                              }}
