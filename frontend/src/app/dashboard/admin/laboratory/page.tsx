...
const MOCK_ORDERS: LabOrder[] = [
  ...
  },
  {
    ...
    testType: "Troponin I (Cardiac)",
    category: "Cardiology",
    orderedBy: "Dr. Michael Chen",
    orderedAt: "2026-06-20 08:14",
    status: "completed",
    result: "Elevated — see notes",
    priority: "stat",
  },
  {
    id: "lab-004",
  ...
  },
  {
    ...
    testType: "X-Ray — Right Femur",
    category: "Radiology",
    orderedBy: "Dr. Arjun Patel",
    orderedAt: "2026-06-20 07:50",
    status: "completed",
    result: "Compound Fracture Grade III",
    priority: "urgent",
  },
  {
    id: "lab-006",
    patientName: "Priya Nair",
    testType: "IgE Antibody Panel",
    category: "Immunology",
    orderedBy: "Dr. Sarah Jenkins",
    orderedAt: "2026-06-20 10:45",
    status: "processing",
    priority: "stat",
  },
  {
    ...
    testType: "Urinalysis + Culture",
    category: "Microbiology",
    orderedBy: "Dr. Elena Rostova",
    orderedAt: "2026-06-20 07:00",
    status: "pending",
    priority: "routine",
  },
  {
...
const PRIORITY_CFG = {
  routine: {
    label: "Routine",
    cls: "bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400",
  },
  urgent: {
    label: "Urgent",
  ...
  stat: {
    label: "STAT",
    cls: "bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20 font-extrabold",
  },
};
...
export default function LaboratoryPage() {
  ...
  const filtered = orders.filter((o) => {
    const ms =
      ...
      o.testType.toLowerCase().includes(search.toLowerCase());
  ...
  const handleComplete = (id: string) => {
    setOrders((prev) =>
      prev.map((o) =>
        o.id === id
          ? { ...o, status: "completed", result: "Results ready" }
          : o,
      ),
    );
    ...
    <div className="space-y-7">
      ...
      <div>
        <div className="flex items-center gap-2 text-[10px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-widest mb-1">
          ...
          <span className="text-rose-500">Laboratory</span>
        </div>
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-rose-500/10 flex items-center justify-center">
            <MdOutlineScience className="h-5 w-5 text-rose-600 dark:text-rose-500 " />
          </div>
          Laboratory Orders
        </h1>
        <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 ml-12">
          Track all diagnostic test orders, processing status, and results.
        </p>
      </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
              {filtered.map((o) => {
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-4 text-[10px] font-bold text-slate-400 font-mono">
                      {o.id}
                    </td>
                    <td className="px-5 py-4 text-xs font-bold text-slate-800 dark:text-zinc-100 whitespace-nowrap">
                      {o.patientName}
                    </td>
                    <td className="px-5 py-4 text-xs text-slate-600 dark:text-zinc-300 max-w-[180px]">
                      {o.testType}
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`text-[10px] font-bold px-2 py-1 rounded-lg ${pc.cls}`}
                      >
                        {pc.label}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-xs text-slate-600 dark:text-zinc-300 whitespace-nowrap">
                      {o.orderedBy}
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1.5 rounded-lg ${sc.color} ${sc.bg} border ${sc.border}`}
                      >
                        <span
                    ...
                    <td className="px-5 py-4 text-xs text-slate-500 dark:text-zinc-400 max-w-[160px] truncate">
                      {o.result || "—"}
                    </td>
                    <td className="px-5 py-4">
                      {(o.status === "pending" ||
                        ...
                        <button
                          ...
                          className="text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 cursor-pointer transition-all"
                        >
                          Mark Complete
                        </button>
                      )}
