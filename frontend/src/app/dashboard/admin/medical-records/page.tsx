    ...
    <div className="space-y-7">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-[10px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-widest mb-1">
            ...
            <span className="text-rose-500">Medical Records</span>
          </div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-rose-500/10 flex items-center justify-center">
              <MdOutlineMedicalInformation className="h-5 w-5 text-rose-600 dark:text-rose-500 " />
            </div>
            Medical Records
          </h1>
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 ml-12">
            Patient diagnoses, prescriptions, and clinical notes across all
            visits.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          ...
          },
          {
            ...
            value: new Set(records.map((r) => r.patientName)).size,
      ...
      {filtered.length > 0 ? (
                ...
                <div className="px-6 py-5 flex items-start gap-4">
                  ...
                  {record.patientPhoto ? (
                    <img
                      ...
                      className="h-12 w-12 rounded-full object-cover border-2 border-white dark:border-zinc-700 shadow-sm shrink-0"
                    />
                  ) : (
                    <div
                      className={`h-12 w-12 rounded-full bg-linear-to-br ${gradient} flex items-center justify-center text-white text-sm font-extrabold shrink-0 shadow-sm`}
                    >
                    ...
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1 flex-wrap">
                          <span className="text-xs font-bold text-rose-600 dark:text-rose-500  bg-rose-500/10 px-2 py-0.5 rounded-lg border border-rose-500/20">
                            {record.diagnosis}
                          </span>
                        ...
                        <div className="text-xs text-slate-500 dark:text-zinc-400 mt-1.5 line-clamp-1">
                          {record.medications}
                        </div>
                      </div>
                ...
                {isExpanded && (
                  <div className="border-t border-slate-100 dark:border-zinc-800 px-6 py-5 bg-slate-50/50 dark:bg-zinc-800/20 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div>
                        <div className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 dark:text-zinc-500 mb-2">
                          Diagnosis
                        </div>
                        <div className="text-sm text-slate-700 dark:text-zinc-200 font-semibold">
                          {record.diagnosis}
                        </div>
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-600 dark:text-zinc-300 leading-relaxed bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-xl p-3">
                        {record.notes}
                      </div>
                    </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-dashed border-slate-300 dark:border-zinc-700 rounded-2xl py-20 flex flex-col items-center gap-4 text-center">
          ...
          <button
            onClick={() => setSearch("")}
            className="text-xs font-bold text-rose-600 border border-rose-500/30 px-4 py-2 rounded-md hover:bg-rose-50 transition-all cursor-pointer"
          >
            Clear Search
          </button>
        </div>
