...
> = {
  ...
  maintenance: {
    ...
    bg: "bg-slate-200/50 dark:bg-zinc-800/50",
...
export default function BedManagementPage() {
  ...
  const patientsList =
    typeof window !== "undefined"
      ? db.getUsers().filter((u) => u.role === "patient")
      : [];
  ...
  const handleAssignSubmit = (e: React.FormEvent) => {
    ...
    updateBeds(updated);
    toast.success(
      `Bed ${selectedBed.bedNo} successfully assigned to ${finalName}.`,
    );
    setSelectedBed(null);
  };
    ...
    <div className="space-y-7">
      ...
      <div>
        <div className="flex items-center gap-2 text-[10px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-widest mb-1">
          ...
          <span className="text-rose-500">Bed Management</span>
        </div>
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-rose-500/10 flex items-center justify-center">
            <MdOutlineBedroomParent className="h-5 w-5 text-rose-600 dark:text-rose-500 " />
          </div>
          Bed Management
        </h1>
        <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 ml-12">
          Real-time ward occupancy and bed allocation across all departments.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          <div
            key={s.label}
            className={`bg-white dark:bg-zinc-900 border ${s.border} rounded-2xl p-5 flex flex-col gap-1 hover:shadow-md transition-shadow`}
          >
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <span
            className={`text-sm font-extrabold ${occupancyPct > 80 ? "text-rose-600" : "text-emerald-600"}`}
          >
            {occupancyPct}%
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-5">
        {filtered.map((bed) => {
          ...
          return (
            <div
              key={bed.id}
              className={`bg-white dark:bg-zinc-900 border ${sc.border} rounded-2xl p-5 flex flex-col gap-4 hover:shadow-lg transition-all justify-between relative overflow-hidden group shadow-sm hover:-translate-y-0.5`}
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-[15px] font-extrabold text-slate-800 dark:text-zinc-100 tracking-tight">
                    {bed.bedNo}
                  </span>
                  <span
                    className={`inline-flex items-center gap-1.5 text-[13px] font-bold px-3 py-1 rounded-full ${sc.color} ${sc.bg} border ${sc.border}`}
                  >
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`h-11 w-11 rounded-xl ${sc.bg} flex items-center justify-center ${sc.color} transition-transform group-hover:scale-110 duration-200 shrink-0`}
                  >
                    {WARD_ICONS[bed.ward]}
                  </div>
                  <div className="min-w-0">
                    <div className="text-[13px] font-extrabold text-slate-400 dark:text-zinc-550 uppercase tracking-wider">
                      {bed.ward} Ward
                    </div>
                    {bed.patient ? (
                      <div className="text-[15px] font-bold text-slate-800 dark:text-zinc-150 mt-0.5 truncate max-w-[130px]">
                        {bed.patient}
                      </div>
                    ) : (
                      <div className="text-[15px] font-semibold text-slate-450 dark:text-zinc-650 mt-0.5 italic">
                        Empty
                      </div>
                    )}
                    {bed.admittedSince && (
                      <div className="text-[13px] font-medium text-slate-400 mt-0.5">
                        Since {bed.admittedSince}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleAssignSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-650 dark:text-zinc-450">
                  Select Patient
                </label>
                <select
