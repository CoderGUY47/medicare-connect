...
export default function PatientAppointmentsPage() {
  ...
  const handleCancel = (aptId: string) => {
    ...
    db.setAppointments(
      db
        ...
        .map((a) =>
          a.id === aptId
            ? { ...a, appointmentStatus: "cancelled" as const }
            : a,
        ),
    );
    loadData();
  ...
  const counts = {
    ...
    cancelled: appointments.filter((a) =>
      ["cancelled", "rejected"].includes(a.appointmentStatus),
    ).length,
  };
        ...
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-rose-500/10 flex items-center justify-center">
              <BsClipboard2Pulse className="h-5 w-5 text-rose-600 dark:text-rose-500 " />
            </div>
            My Appointments
          </h1>
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 ml-12">
            All scheduled and past medical consultations.
          </p>
        </div>
        <Link
          href="/find-doctors"
          className="flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-rose-500/20 shrink-0"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          ...
          return (
            <div
              key={s.label}
              className={`bg-white dark:bg-zinc-900 border ${s.border} rounded-2xl p-4 flex items-center gap-3 hover:shadow-md transition-shadow`}
            >
              <div
      ...
      {filtered.length > 0 ? (
        <div className="space-y-4">
          {filtered.map((apt) => {
            ...
            if (!doc) return null;
            const sc =
              STATUS_CFG[apt.appointmentStatus as AptStatus] ||
              STATUS_CFG.pending;
            const canAct = ["pending", "confirmed"].includes(
              ...
              >
                ...
                <div className="px-5 py-4 flex items-center justify-between gap-4 border-b border-slate-100 dark:border-zinc-800">
                  <div className="flex items-center gap-3 min-w-0">
                    <img
                      ...
                      alt={doc.doctorName}
                      className="h-12 w-12 rounded-xl object-cover border-2 border-white dark:border-zinc-700 shadow-sm shrink-0"
                    />
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] font-bold text-slate-400 font-mono mt-0.5">
                        APT-{apt.id.slice(-6).toUpperCase()}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                    <span
                      className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1.5 rounded-xl ${sc.pill}`}
                    >
                      <span className={`h-1.5 w-1.5 rounded-full ${sc.dot}`} />
                      {sc.label}
                    </span>
                    <span
                      className={`text-[11px] font-bold px-2.5 py-1.5 rounded-xl ${apt.paymentStatus === "paid" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20" : "bg-amber-500/10 text-amber-600 border border-amber-500/20"}`}
                    >
                ...
                <div className="px-5 py-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-slate-50 dark:bg-zinc-800/50 rounded-xl p-3 col-span-2 md:col-span-1">
                    ...
                    <div className="text-xs text-slate-600 dark:text-zinc-300 leading-relaxed line-clamp-2">
                    ...
                    <>
                      ...
                      <button
                        onClick={() => {
                          ...
                          setRescheduleDay(doc.availableDays[0] || "");
                        }}
        ...
        <div className="bg-white dark:bg-zinc-900 border border-dashed border-slate-300 dark:border-zinc-700 rounded-2xl py-20 flex flex-col items-center gap-4 text-center">
          ...
          <Link
            href="/find-doctors"
            className="flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs px-5 py-2.5 rounded-xl transition-all shadow-md shadow-rose-500/20"
          >
            Find a Doctor
          </Link>
        </div>
          ...
          <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between">
              ...
              <button
                ...
                className="p-1.5 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg text-slate-400 transition-colors cursor-pointer"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            <form onSubmit={handleRescheduleSubmit} className="p-6 space-y-5">
              <div className="flex items-center gap-3 bg-rose-50 dark:bg-rose-900/10 rounded-xl p-3">
                <img
                  src={reschedDoc.profileImage}
                  alt={reschedDoc.doctorName}
                  className="h-10 w-10 rounded-full object-cover shrink-0"
                />
                <div>
                  <div className="text-xs font-bold text-slate-800 dark:text-zinc-100">
                    {reschedDoc.doctorName}
                  </div>
                  <div className="text-[10px] text-slate-400">
                    {reschedDoc.specialization}
                  </div>
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={!rescheduleDay || !rescheduleSlot}
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div
                    key={f.label}
                    className="bg-slate-50 dark:bg-zinc-800 rounded-xl p-3"
                  >
                    <div className="text-[9px] font-extrabold uppercase tracking-widest text-slate-400 dark:text-zinc-500 mb-0.5">
                      {f.label}
                    </div>
                ...
                <div className="bg-slate-50 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-700 rounded-xl p-3 space-y-1.5">
                  {prescription.medications
                    ...
                    .map((m: string, i: number) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 text-xs text-slate-700 dark:text-zinc-200"
                      >
