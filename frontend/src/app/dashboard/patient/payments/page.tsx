...
export default function PatientPaymentsPage() {
  ...
  const avgFee =
    payments.length > 0 ? Math.round(totalSpent / payments.length) : 0;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-[10px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-widest mb-1">
            ...
            <span className="text-rose-500">Payments</span>
          </div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 ml-12">
            All consultation fee transactions and receipts.
          </p>
        </div>
      ...
      {payments.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-[11px] font-bold text-rose-600 dark:text-rose-500  bg-rose-500/10 px-3 py-1 rounded-lg">
              2026
            </span>
          </div>
          {mounted && (
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="divide-y divide-slate-100 dark:divide-zinc-800">
            {payments.map((p, i) => (
              <div
                key={p.id}
                className="flex items-center gap-4 px-6 py-4 hover:bg-rose-50/30 dark:hover:bg-zinc-800/40 transition-colors"
              >
                {/* Icon */}
        ...
        <div className="bg-white dark:bg-zinc-900 border border-dashed border-slate-300 dark:border-zinc-700 rounded-2xl py-20 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Payments will appear here after you book a consultation.
            </p>
          </div>
        </div>
      )}
