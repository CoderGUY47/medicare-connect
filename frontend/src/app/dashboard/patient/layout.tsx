        ...
        <nav className="flex-1 py-3 px-3 space-y-0.5">
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`h-4 w-4 shrink-0 ${active ? "text-rose-600 dark:text-rose-500 " : "text-slate-400"}`}
                />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 border-b border-slate-100 dark:border-zinc-900 bg-white dark:bg-zinc-900 flex items-center justify-between px-6 md:px-8 shrink-0">
          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-zinc-400 min-w-0">
            ...
            <span className="hidden sm:inline capitalize truncate">
              {user.role} Dashboard
            </span>
          </div>

          <div className="flex items-center gap-4">
            <span className="text-[11px] font-semibold text-slate-450 dark:text-zinc-500 hidden lg:inline">
              {currentTime}
            </span>
            ...
            {user.photo ? (
              <img
                src={user.photo}
                alt={user.name}
                className="h-8 w-8 rounded-full object-cover border-2 border-rose-500/30 shadow-sm"
              />
            ) : (
