...
export default function PatientOverviewPage() {
  ...
  useEffect(() => {
    ...
    const completed = allApts.filter(
        ...
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between">
            ...
            <Link
              ...
              className="flex items-center gap-1 text-[11px] font-bold text-rose-600 dark:text-rose-500  hover:underline"
            >
                ...
                <div className="flex items-center justify-between pt-1">
                  <div>
                    ...
                    <div className="text-sm font-extrabold text-slate-800 dark:text-zinc-100">
                      ৳{nearestDoc.consultationFee}
                    </div>
                  </div>
                  {(() => {
                    const sc =
                      STATUS_CFG[nearestApt.appointmentStatus as AptStatus] ||
                      STATUS_CFG.pending;
                    return (
                      <span
                        className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-3 py-1.5 rounded-xl ${sc.pill}`}
                      >
                        <span
        ...
        <div className="lg:col-span-3 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="p-5">
            <div className="flex items-center gap-6 mb-5">
              <div>
                <div className="text-2xl font-extrabold text-slate-800 dark:text-zinc-100">
                  ৳{stats.totalPaid}
                </div>
            ...
            <div className="divide-y divide-slate-100 dark:divide-zinc-800">
              {recentApts.map((apt) => {
                ...
                return (
                  <div
                    key={apt.id}
                    className="flex items-center gap-4 px-5 py-4 hover:bg-rose-50/30 dark:hover:bg-zinc-800/40 transition-colors"
                  >
                    {doc?.profileImage ? (
                      <img
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-slate-400 mt-0.5">
                        {doc?.specialization} · {apt.appointmentDate}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1.5 shrink-0">
                      <span
                        className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg ${sc.pill}`}
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${sc.dot}`}
                        />
        ...
        <div className="space-y-4">
              ...
              {[
                {
                  ...
                  icon: Stethoscope,
                  color:
                    "text-rose-600 bg-rose-500/10 border-rose-500/20 hover:bg-rose-500/20",
                },
                {
                ...
                },
                {
                  ...
                  icon: FiActivity,
                  color:
                    "text-violet-600 bg-violet-500/10 border-violet-500/20 hover:bg-violet-500/20",
                },
              ].map((action) => {
          ...
          <div className="bg-linear-to-br from-rose-500/10 via-pink-500/5 to-transparent border border-rose-500/15 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-extrabold text-rose-600 dark:text-rose-500  uppercase tracking-wider">
                Health Tip
              </span>
            </div>
            <p className="text-xs text-slate-600 dark:text-zinc-300 leading-relaxed">
              Stay on top of your health. Regular check-ups help catch issues
              early. Your last completed visit was with{" "}
              {recentApts.find((a) => a.appointmentStatus === "completed")
                ? doctorsMap[
                    recentApts.find((a) => a.appointmentStatus === "completed")!
                      .doctorId
                  ]?.doctorName || "a specialist"
                : "a specialist"}
              .
            </p>
            <Link
              href="/find-doctors"
              className="inline-flex items-center gap-1.5 mt-3 text-[11px] font-bold text-rose-600 dark:text-rose-500  hover:underline"
            >
