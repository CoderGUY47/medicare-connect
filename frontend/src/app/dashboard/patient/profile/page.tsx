...
export default function PatientProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"info" | "security" | "stats">(
    "info",
  );
      ...
      <div>
        ...
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-rose-500/10 flex items-center justify-center">
            <MdOutlineManageAccounts className="h-5 w-5 text-rose-600 dark:text-rose-500 " />
          </div>
          My Profile
        </h1>
        ...
        <div className="px-6 pb-6">
          ...
          <div className="flex items-end gap-5">
            ...
            <div className="pb-1 min-w-0">
              <div className="text-xl font-extrabold text-slate-900 dark:text-zinc-50 truncate">
                {user?.name}
              </div>
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className="text-xs font-bold text-rose-600 bg-rose-500/10 px-2.5 py-1 rounded-lg border border-rose-500/20 uppercase tracking-wider">
                  Patient
                </span>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-5">
            {[
              ...
              <div
                key={s.label}
                className="bg-slate-50 dark:bg-zinc-800/50 rounded-xl p-3 text-center"
              >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
              ...
              <div className="flex-1">
                <label className="text-xs font-bold text-slate-600 dark:text-zinc-400 block mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <FaLink className="h-3 w-3" /> Profile Photo URL
                  </span>
                </label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 dark:text-zinc-400 block mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <FaUser className="h-3 w-3" /> Full Name
                  </span>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 dark:text-zinc-400 block mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <FaPhone className="h-3 w-3" /> Phone Number
                  </span>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 dark:text-zinc-400 block mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <FaEnvelope className="h-3 w-3" /> Email Address
                  </span>
                </label>
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 dark:text-zinc-400 block mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <FaIdCard className="h-3 w-3" /> Account Role
                  </span>
                </label>
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-zinc-400 block mb-2">
                <span className="flex items-center gap-1.5">
                  <FaVenusMars className="h-3 w-3" /> Gender
                </span>
              </label>
          ...
          <div className="p-6 space-y-4">
            {[
              ...
              >
                ...
                <div
                  className={`text-xs font-bold px-3 py-1.5 rounded-xl capitalize font-mono ${item.bg} ${item.color}`}
                >
                  {item.value}
                </div>
              </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="p-6 space-y-4">
            {[
              ...
              {
                label: "Total Amount Paid (USD)",
                value: `$${stats.totalSpent}`,
                color: "text-blue-600 dark:text-blue-400",
                icon: "💳",
              },
              {
                label: "Doctor Reviews Written",
                value: stats.reviews,
                color: "text-amber-600 dark:text-amber-400",
                icon: "⭐",
              },
