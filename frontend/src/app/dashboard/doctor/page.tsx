...
export default function DoctorOverviewPage() {
  ...
  const [todayBookings, setTodayBookings] = useState<
    (Appointment & { patientName?: string; patientPhoto?: string })[]
  >([]);
  const [chartData, setChartData] = useState<
    { day: string; consults: number }[]
  >([]);
  ...
  const metrics = [
    ...
    },
    {
      ...
      desc: "Patient feedback submissions",
      icon: Heart,
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((m) => {
          ...
          return (
            <div
              key={m.label}
              className={`bg-white dark:bg-zinc-900 border ${m.border} rounded-2xl p-5 flex items-center gap-4 hover:shadow-md transition-shadow`}
            >
              <div
                className={`h-11 w-11 rounded-xl ${m.bg} flex items-center justify-center shrink-0`}
              >
                <Icon className={`h-5 w-5 ${m.color}`} />
              </div>
              <div>
                <div
                  className={`text-2xl font-extrabold text-slate-800 dark:text-zinc-150`}
                >
                  {m.value}
                </div>
                <div className="text-[11px] font-bold text-slate-500 dark:text-zinc-400 mt-0.5">
                  {m.label}
                </div>
              </div>
            </div>
            ...
            <div className="divide-y divide-slate-100 dark:divide-zinc-800">
              {todayBookings.length > 0 ? (
                todayBookings.map((apt) => (
                  <div
                    key={apt.id}
                    className="p-4 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-zinc-800/40 transition-colors"
                  >
                    {apt.patientPhoto ? (
                      <img
                    ...
                    <span
                      className={`px-2 py-0.5 border text-[8px] font-extrabold uppercase rounded-lg tracking-wider shrink-0 ${statusCls[apt.appointmentStatus] || "bg-slate-100 text-slate-500 border-slate-200"}`}
                    >
          ...
          {recentReviews.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {recentReviews.map((rev) => (
                <div
                  key={rev.id}
                  className="border border-slate-150 dark:border-zinc-800 p-4 rounded-xl space-y-2 hover:border-emerald-500/20 transition-all"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-0.5 bg-amber-500/10 px-2 py-0.5 rounded-lg border border-amber-500/10">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          key={i}
                          className={`h-3 w-3 ${i < rev.rating ? "fill-amber-400 stroke-amber-400" : "stroke-slate-350 dark:stroke-zinc-700"}`}
                        />
                      ))}
                    </div>
                  </div>
                  <p className="text-xs text-slate-600 dark:text-zinc-350 italic leading-relaxed">
            ...
            <div className="py-12 border border-dashed border-slate-200 dark:border-zinc-800 rounded-xl text-center text-slate-400 dark:text-zinc-500">
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">
                Reviews appear once patients complete visits.
              </p>
            </div>
          )}
        </div>
