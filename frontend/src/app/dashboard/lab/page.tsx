    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">
          Laboratory specimen processing, diagnostic request status, and queue
          telemetry
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => {
          ...
          return (
            <div
              key={s.label}
              className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 p-4 rounded-2xl flex items-center gap-4 shadow-sm"
            >
              <div
                className={`${s.bg} ${s.color} p-3 rounded-xl flex items-center justify-center`}
              >
              ...
              <div>
                ...
                <span className="text-xl font-extrabold text-slate-800 dark:text-zinc-150 tracking-tight mt-0.5 block">
                  {s.value}
                </span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-5 shadow-sm lg:col-span-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-slate-800 dark:text-zinc-150">
              Specimen Testing Queue
            </h3>
            <span className="text-[10px] font-bold text-rose-600 bg-rose-500/5 px-2 py-0.5 rounded-full">
              Section B-1
            </span>
          </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody>
                {specimens.map((s) => (
                  ...
                  >
                    <td className="py-3 font-mono font-bold text-rose-600 dark:text-rose-500 ">
                      {s.id}
                    </td>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-5 shadow-sm lg:col-span-4 flex flex-col justify-between">
          <div>
            <h3 className="text-sm font-bold text-slate-800 dark:text-zinc-150 mb-3.5">
              Device Diagnostics
            </h3>
            <div className="space-y-3">
              {[
                ...
                {
                  name: "Chemistry Analyzer #2",
                  type: "Cobas c311",
                  status: "Idle",
                  ok: true,
                },
                {
                  name: "Coagulation System #1",
                  type: "ACL TOP 350",
                  status: "Active",
                  ok: true,
                },
                ...
                <div
                  key={idx}
                  className="flex gap-3 items-start border-b border-slate-50 dark:border-zinc-800 pb-2.5 last:border-b-0"
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-450 dark:text-zinc-500 leading-normal">
                      ...
                      <strong
                        className={
                          device.ok ? "text-slate-500" : "text-amber-500"
                        }
                      >
                        {device.status}
                      </strong>
