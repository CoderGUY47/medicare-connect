...
export default function DoctorDetailPage() {
  ...
  if (!doctor) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center space-y-5">
        ...
        <h2 className="font-extrabold text-base uppercase text-slate-800 dark:text-zinc-100 tracking-wider">
          Doctor Not Found
        </h2>
        <p className="text-xs text-slate-500 dark:text-zinc-400">
          This profile registry entry does not exist or was removed.
        </p>
        <button
          onClick={() => router.push("/find-doctors")}
          className="bg-rose-600 hover:bg-rose-700 px-6 py-2.5 rounded-[8px] text-xs font-bold text-white uppercase tracking-wider transition-all shadow-lg shadow-rose-600/10 active:scale-[0.98]"
        >
          Back to Directory
        </button>
      </div>
  ...
  const isSlotBooked = (slot: string) =>
    bookedSlots[selectedDateStr]?.includes(slot) || false;
  ...
  const handleBookingSubmit = (e: React.FormEvent) => {
    ...
    if (user.role !== "patient") {
      setErrorMsg("Only patient accounts can book appointments.");
      return;
    }
    if (!selectedDay || !selectedSlot) {
      setErrorMsg("Please select a date and a time slot.");
      return;
    }
    if (!symptoms.trim()) {
      setErrorMsg("Please describe your symptoms so the doctor can prepare.");
      return;
    }
    router.push(
      `/checkout?${new URLSearchParams({ doctorId: doctor.id, date: selectedDateStr, time: selectedSlot, symptoms: symptoms.trim() }).toString()}`,
    );
  };
  ...
  return (
    <div className="container px-4 py-8 sm:px-6 lg:px-8 space-y-6">
      ...
      <button
        onClick={() => router.push("/find-doctors")}
        className="flex items-center gap-1.5 text-xs font-bold text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-100 transition-colors uppercase tracking-wider bg-transparent border-none cursor-pointer"
      >
        ...
        <div className="lg:col-span-8 space-y-5">
          ...
          <div className="border border-slate-100 dark:border-zinc-800 bg-card rounded-[10px] shadow-sm overflow-hidden">
            ...
            <div className="p-6 flex flex-col sm:flex-row gap-6 items-center sm:items-start text-center sm:text-left">
              <img
                ...
                className="h-28 w-28 object-cover rounded-2xl border border-slate-200 dark:border-zinc-800 flex-shrink-0 shadow-sm"
              />
              <div className="space-y-4 flex-1">
                <div>
                  <span className="inline-block border border-rose-500/25 bg-rose-500/5 px-2.5 py-1 text-[10px] font-bold text-rose-600 dark:text-rose-500  uppercase tracking-widest rounded-lg mb-2">
                    {doctor.specialization}
                  </span>
                  <h1 className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-zinc-50">
                    {doctor.doctorName}
                  </h1>
                ...
                <div className="border border-slate-100 dark:border-zinc-800/80 bg-slate-50/50 dark:bg-zinc-950/20 p-4 rounded-xl">
                  ...
                  <div className="text-xs text-slate-700 dark:text-zinc-300 leading-relaxed font-medium">
            ...
            <div className="border-t border-slate-100 dark:border-zinc-800 grid grid-cols-3 divide-x divide-slate-100 dark:divide-zinc-800 bg-slate-50/30 dark:bg-zinc-900/10">
              {[
                ...
                <div key={s.label} className="px-4 py-4 text-center">
                  ...
                  <div className="text-base font-extrabold text-rose-600 dark:text-rose-500  mt-1">
                    {s.value}
                  </div>
                </div>
              ))}
          ...
          <div className="border border-slate-100 dark:border-zinc-800 bg-card rounded-[10px] shadow-sm overflow-hidden">
            <div className="border-b border-slate-100 dark:border-zinc-800 px-6 py-4 flex items-center gap-2 bg-slate-50/50 dark:bg-zinc-900/50">
              ...
              <span className="text-xs uppercase tracking-wider text-slate-800 dark:text-zinc-100 font-extrabold">
                About the Doctor
              </span>
            </div>
            <div className="p-6">
              <p className="text-xs md:text-sm text-slate-500 dark:text-zinc-400 leading-relaxed">
                {doctor.doctorName} is a highly committed specialist with{" "}
                {doctor.experience} years of clinical excellence practicing at{" "}
                {doctor.hospitalName}. Experienced in addressing complex cases,
                prescribing personalized pathways, and guiding patients toward
                sustained recovery.
              </p>
            </div>
          </div>
          ...
          <div className="border border-slate-100 dark:border-zinc-800 bg-card rounded-[10px] shadow-sm overflow-hidden">
            <div className="border-b border-slate-100 dark:border-zinc-800 px-6 py-4 flex items-center justify-between bg-slate-50/50 dark:bg-zinc-900/50">
              ...
              <span className="flex items-center gap-1 text-xs font-extrabold text-amber-500 bg-amber-500/5 border border-amber-500/10 px-2.5 py-1 rounded-lg">
                ★ {avgRating} Avg Rating
              </span>
            </div>
            ...
            {reviews.length > 0 ? (
                ...
                {reviews.map((rev) => (
                  <div key={rev.id} className="flex gap-4 p-6 items-start">
                    <img
                      ...
                      alt={rev.patientName}
                      className="h-10 w-10 object-cover rounded-full border border-slate-200 dark:border-zinc-800 flex-shrink-0 shadow-sm"
                    />
              ...
              <div className="py-12 text-center bg-transparent">
                ...
                <p className="text-[10px] text-slate-400 dark:text-zinc-500 uppercase font-extrabold tracking-wider">
          ...
          <form onSubmit={handleBookingSubmit} className="p-5 space-y-5">
            ...
            <div className="flex items-center justify-between border border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-950/20 px-4 py-3 rounded-xl">
              <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-555">
                <i className="fa-solid fa-bangladeshi-taka-sign text-xs text-rose-500 mr-0.5"></i>{" "}
                Consultation Fee
              </span>
              <span className="text-lg font-extrabold text-rose-600 dark:text-rose-500 ">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {doctor.availableDays.map((day) => (
                  ...
                  >
                    {day}
                    <span className="block text-[9px] opacity-75 font-normal normal-case mt-0.5">
                      {getNextDateForDay(day).split("-").slice(1).join("/")}
                    </span>
                  </button>
                ))}
              </div>
            ...
            {selectedDay && (
              <div className="space-y-2">
                <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">
                  <Clock className="h-4 w-4 text-rose-500" /> 2. Choose Time
                  Slot
                </div>
                <div className="grid grid-cols-2 gap-2">
                      ...
                      <button
                        ...
                        className={`py-2 text-center border rounded-lg text-xs font-bold transition-all cursor-pointer ${
                          selectedSlot === slot
                            ...
                            : booked
                              ? "border-slate-100 dark:border-zinc-900 bg-slate-50/50 dark:bg-zinc-800/10 text-slate-300 dark:text-zinc-650 line-through cursor-not-allowed"
            ...
            <div className="space-y-2">
              ...
              <textarea
                rows={4}
                required
                placeholder="Describe any symptoms, conditions or concerns..."
                value={symptoms}
            ...
            <button
              type="submit"
              className="w-full bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold uppercase py-3.5 rounded-lg transition-all flex items-center justify-center gap-1.5 shadow-lg shadow-rose-600/10 active:scale-[0.98] cursor-pointer"
            >
              <CheckCircle2 className="h-4 w-4" /> Proceed to Payment
            </button>

            {!user && (
              <p className="text-[10px] text-center text-slate-400 dark:text-zinc-500 font-semibold">
                * You will be asked to sign in to Medi-Doc to complete checkout.
              </p>
            )}
          </form>
        </div>
