...
export default function FindDoctorsPage() {
  ...
  const fetchDoctors = useCallback(async () => {
    ...
    try {
      ...
      const specs = [
        "all",
        ...Array.from(new Set(verified.map((d) => d.specialization))),
      ].sort((a, b) =>
      ...
      const specs = [
        "all",
        ...Array.from(new Set(localDocs.map((d) => d.specialization))),
      ].sort((a, b) =>
        a === "all" ? -1 : b === "all" ? 1 : a.localeCompare(b),
      );
      setSpecializations(specs);
    ...
    <div className="min-h-screen container px-4 py-12 sm:px-6 lg:px-8 space-y-8 pb-24">
      ...
      <ScrollAnimate>
        <div className="space-y-2 border-b border-slate-100 dark:border-zinc-800/80 pb-6">
          <div className="inline-flex items-center gap-1.5 rounded-full bg-rose-500/10 px-3 py-1 text-xs font-semibold text-rose-600 dark:text-rose-500 ">
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400">
            Search and book medical consultations with verified specialists in
            Bangladesh.
          </p>
          {/* Live data badge */}
          <div className="flex items-center gap-2">
            {isLoading ? (
              ...
              <span className="inline-flex items-center gap-1.5 text-[11px] text-amber-500 bg-amber-500/10 border border-amber-500/20 rounded-full px-2.5 py-1">
                <WifiOff className="h-3 w-3" /> Offline mode — showing local
                data
              </span>
            ) : (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-xs border-b border-slate-100 dark:border-zinc-800/80 pb-4 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      ...
                      <strong className="text-slate-900 dark:text-white font-bold">
                        {filteredDoctors.length > 0 ? indexOfFirst + 1 : 0}–
                        {Math.min(indexOfLast, filteredDoctors.length)}
                      </strong>{" "}
                      of{" "}
                      <strong className="text-slate-900 dark:text-white font-bold">
              ...
              <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 text-[10px] font-bold rounded-[6px] transition-all cursor-pointer border-none ${
                      viewMode === "card"
                        ...
                        : "text-slate-500 hover:text-slate-800 dark:text-zinc-400 bg-transparent"
                    }`}
                  >
                    Card View
                  </button>
                ...
                <div className="flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 px-2.5 py-1 text-xs">
                  ...
                  >
                    <option value="rating-desc" className="bg-card">
                      Rating: High to Low
                    </option>
                    <option value="experience-desc" className="bg-card">
                      Exp: High to Low
                    </option>
                    <option value="fee-asc" className="bg-card">
                      Fee: Low to High
                    </option>
                    <option value="fee-desc" className="bg-card">
                      Fee: High to Low
                    </option>
                  </select>
                </div>
            ...
            {isLoading ? (
                        ...
                        <div className="flex-1 flex flex-col justify-between min-w-0">
                          <div className="space-y-2">
                            <div className="flex flex-wrap items-center gap-1.5">
                              <span className="inline-flex items-center rounded-md bg-rose-50 dark:bg-rose-500/10 px-2 py-0.5 text-[10px] font-semibold text-rose-600 dark:text-rose-500  border border-rose-100 dark:border-rose-500/10">
                                {doc.specialization}
                              </span>
                          ...
                          <div className="border-t border-slate-100 dark:border-zinc-800 pt-3 mt-3 flex items-center justify-between gap-4">
                            <div>
                              ...
                              <div className="text-slate-900 dark:text-white font-extrabold text-sm mt-0.5 flex items-center">
                                <i className="fa-solid fa-bangladeshi-taka-sign text-xs mr-0.5" />
                                {doc.consultationFee}
                              </div>
                            </div>
                            <Link
                      ...
                      {currentDoctors.map((doc) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-3.5 px-5 font-semibold text-rose-600 dark:text-rose-500 ">
                              {doc.specialization}
                            </td>
                            <td className="py-3.5 px-5 font-medium text-slate-500 dark:text-zinc-400">
                              {doc.hospitalName}
                            </td>
                            <td className="py-3.5 px-5 font-bold text-slate-700 dark:text-zinc-300">
                              {doc.experience} Yrs
                            </td>
                            ...
                            <td className="py-3.5 px-5 text-right font-extrabold text-slate-900 dark:text-zinc-100">
                              <i className="fa-solid fa-bangladeshi-taka-sign text-[11px] mr-0.5" />
                              {doc.consultationFee}
                            </td>
              ...
              <div className="rounded-[10px] border border-dashed border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-12 text-center space-y-4">
                ...
                <p className="text-xs text-slate-500 dark:text-zinc-400 max-w-xs mx-auto">
                  We couldn&apos;t find any certified specialist matching your
                  search.
                </p>
                <button
                  onClick={clearAllFilters}
