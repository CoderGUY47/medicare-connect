...
export default function FindDoctorsPage() {
  ...
  useEffect(() => {
    ...
    // Sorting
    list.sort((a, b) => {
      // Mock ratings since all are 4.9 in mockDb seeds
      const aRating = 4.9;
      const bRating = 4.9;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800/80 pb-3">
            ...
            {(searchTerm || selectedSpecialization !== 'all' || sortBy !== 'rating-desc') && (
              <button
                ...
                className="text-xs text-rose-500 font-semibold hover:text-rose-600 flex items-center gap-0.5 cursor-pointer"
              >
        ...
        <ScrollAnimate className="lg:col-span-9">
          <div className="space-y-6">
            <div className="flex items-center justify-between text-xs border-b border-slate-100 dark:border-zinc-800/80 pb-3">
            <div className="flex items-center gap-3 text-slate-500 dark:text-zinc-400">
              ...
              {debouncedSearch && (
                <div className="flex items-center gap-1.5 border-l border-slate-200 dark:border-zinc-800 pl-3">
                  <span className="text-slate-450 dark:text-zinc-550">Query:</span>
          ...
          {currentDoctors.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {currentDoctors.map((doc) => (
                <div
                  key={doc.id}
                  className="group rounded-md border border-slate-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 p-4 flex flex-col sm:flex-row gap-5"
                >
                  {/* Photo Container */}
                  <div className="relative w-full sm:w-32 h-44 sm:h-32 rounded-md overflow-hidden bg-slate-50 dark:bg-zinc-800 shrink-0 border border-slate-100 dark:border-zinc-800/50">
                    <img
                  ...
                  <div className="flex-1 flex flex-col justify-between min-w-0">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-1 text-xs text-slate-400 dark:text-zinc-500">
                        <MapPin className="h-3.5 w-3.5 text-slate-450 dark:text-zinc-600 shrink-0" />
                    ...
                    <div className="border-t border-slate-100 dark:border-zinc-850 pt-3 mt-3 flex items-center justify-between gap-4">
                      ...
                      <Link
                        href={`/doctors/${doc.id}`}
                        className="bg-rose-500 hover:bg-rose-600 text-white font-semibold px-3.5 py-2 rounded-md text-xs transition-all active:scale-[0.98] shadow-sm hover:shadow-md shadow-rose-500/10 cursor-pointer"
                      >
                        Book Now
                      </Link>
                    </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-6">
              <button
                ...
                className="p-2 rounded-[8px] border border-slate-200 dark:border-zinc-800 text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-800 disabled:opacity-50 transition-colors cursor-pointer"
