...
import { Mail, Phone, MapPin, Send, MessageSquare, ShieldAlert, HeartPulse, Clock } from 'lucide-react';
    ...
    <div className="container px-4 py-12 sm:px-6 lg:px-8 space-y-12 pb-24">
      ...
      <ScrollAnimate>
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Contact Our Care Teams</h1>
          <p className="text-base text-slate-600 dark:text-zinc-300 leading-relaxed">
            Have questions about booking appointments, hospital admission, diagnostic services, or medical records? We are here to help. Reach out using any of the channels below or send a secure direct inquiry message.
          </p>
        </div>
      </ScrollAnimate>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <ScrollAnimate className="lg:col-span-5">
          <div className="space-y-6">
            <div className="space-y-4">
              {contactCards.map((c) => (
                <div key={c.label} className={`p-5 rounded-[10px] border flex gap-4 ${c.bg}`}>
                  <div className="p-3 h-fit rounded-lg bg-white dark:bg-zinc-900 shadow-sm shrink-0">
                    {c.icon}
                  </div>
                  <div className="space-y-1.5">
                    <span className="text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest">{c.label}</span>
                    <div className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">{c.value}</div>
                    <p className="text-xs text-slate-500 dark:text-zinc-400">{c.sub}</p>
                    <p className="text-xs text-slate-600 dark:text-zinc-300 leading-relaxed font-medium pt-1">{c.desc}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="p-6 rounded-[10px] border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 space-y-4 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white">Central Campus Address</h3>
                  <p className="text-xs text-slate-500 dark:text-zinc-400">Addis Ababa, Ethiopia</p>
                </div>
              </div>
              <p className="text-sm text-slate-600 dark:text-zinc-300 leading-relaxed">
                Medi-Doc Tertiary Care Hospital, Bole Sub-City, Road 3, Ring Road Gate 4, Addis Ababa.
              </p>
              <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-zinc-400 border-t border-slate-100 dark:border-zinc-800/80 pt-3">
                ...
                <span>Outpatient Visiting Hours: 08:00 AM – 08:00 PM Daily</span>
              </div>
            </div>
          </div>
        </ScrollAnimate>
        ...
        <ScrollAnimate className="lg:col-span-7">
          ...
          <div className="border-b border-slate-100 dark:border-zinc-800/80 px-6 py-4 flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Send Secure Inquiry</h2>
          </div>

          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-600 dark:text-zinc-300">Your Full Name <span className="text-rose-500">*</span></label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-600 dark:text-zinc-300">Email Address <span className="text-rose-500">*</span></label>
                <input
                  type="email"
                  required
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-600 dark:text-zinc-300">Target Department <span className="text-rose-500">*</span></label>
                <select
                  value={department}
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-600 dark:text-zinc-300">Subject (Optional)</label>
                <input
                  type="text"
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-600 dark:text-zinc-300">Message / Inquiry Details <span className="text-rose-500">*</span></label>
              <textarea
                required
                rows={5}
